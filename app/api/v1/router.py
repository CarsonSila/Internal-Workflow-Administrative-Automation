...
from app.api.deps import get_dataframes, get_file_path
...
@router.get("/overview/metrics", response_model=MetricResponse, tags=["overview"])
def get_overview_metrics(dfs = Depends(get_dataframes)):
    if dfs.beneficiaries_df.empty or dfs.source_records_df.empty:
        return MetricResponse(total_records=0, trusted_identities=0, pending_reviews=0, data_health=0.0)

    total_records = len(dfs.source_records_df)
    trusted_identities = len(dfs.beneficiaries_df)

    pending_reviews = len(dfs.beneficiaries_df[dfs.beneficiaries_df["status"].isin(["Review", "Conflict", "Needs review"])])

    avg_health = float(dfs.source_records_df["data_quality_score"].mean())
    if math.isnan(avg_health):
        avg_health = 0.0

    potential_duplicates = max(total_records - trusted_identities, 0)
    records_merged = int((dfs.audit_logs_df["event"].astype(str) == "Master Records Merged").sum()) if not dfs.audit_logs_df.empty else 0
...
@router.get("/overview/charts", response_model=OverviewChartsResponse, tags=["overview"])
def get_overview_charts(dfs = Depends(get_dataframes)):
    ...
    duplicate_stats = []
    if not dfs.source_records_df.empty:
        for program in ["Scholarship", "Plus", "Vocational", "Tech"]:
            program_df = dfs.source_records_df[dfs.source_records_df["program"] == program]
    ...
    if not dfs.beneficiaries_df.empty:
        for programs in dfs.beneficiaries_df["programs"].fillna("").astype(str):
    ...
    if not dfs.beneficiaries_df.empty:
        for confidence in pd.to_numeric(dfs.beneficiaries_df["confidence"], errors="coerce").fillna(0):
    ...
    if not dfs.audit_logs_df.empty:
        merges = dfs.audit_logs_df[dfs.audit_logs_df["event"].astype(str) == "Master Records Merged"]
        merge_counts.update(merges["date"].astype(str).tolist())
    scan_counts = Counter()
    if not dfs.source_records_df.empty:
        scan_counts.update(dfs.source_records_df["ingested_at"].astype(str).str[:10].tolist())
...
@router.get("/overview/programs", response_model=List[ProgramMetrics], tags=["overview"])
def get_program_metrics(dfs = Depends(get_dataframes)):
    if dfs.source_records_df.empty:
        return []

    prog_counts = dfs.source_records_df.groupby("program").size().to_dict()
...
@router.get("/identities", response_model=List[IdentityItem], tags=["identities"])
def get_identities(
    ...
    dfs = Depends(get_dataframes)
):
    if dfs.beneficiaries_df.empty:
        return []

    df = dfs.beneficiaries_df.copy()
...
@router.get("/beneficiaries/{beneficiary_id}", response_model=BeneficiaryProfile, tags=["identities"])
def get_beneficiary_profile(beneficiary_id: str, dfs = Depends(get_dataframes)):
    if dfs.beneficiaries_df.empty:
        raise HTTPException(status_code=404, detail="Beneficiary database is empty")

    b_records = dfs.beneficiaries_df[dfs.beneficiaries_df["beneficiary_id"] == beneficiary_id]
    if b_records.empty:
...
@router.get("/beneficiaries/{beneficiary_id}/timeline", response_model=List[TimelineEvent], tags=["identities"])
def get_beneficiary_timeline(beneficiary_id: str, dfs = Depends(get_dataframes)):
    timeline = []

    if not dfs.source_records_df.empty:
        b_sources = dfs.source_records_df[dfs.source_records_df["beneficiary_id"] == beneficiary_id]
    ...
    if not dfs.audit_logs_df.empty:
        b_audits = dfs.audit_logs_df[dfs.audit_logs_df["detail"].astype(str).str.contains(beneficiary_id)]
        for _, audit in b_audits.iterrows():
            timeline.append(TimelineEvent(
                date=str(audit["date"]),
                time=str(audit["time"])[:5],
...
@router.get("/duplicates/compare", response_model=DuplicateCompareResponse, tags=["duplicates"])
def compare_records(
    ...
    rec_a_df = dfs.source_records_df[dfs.source_records_df["record_id"] == record_a]
    rec_b_df = dfs.source_records_df[dfs.source_records_df["record_id"] == record_b]
...
@router.post("/duplicates/resolve", tags=["duplicates"])
def resolve_duplicates(
    ...
    rec_a_df = dfs.source_records_df[dfs.source_records_df["record_id"] == rec_a_id]
    rec_b_df = dfs.source_records_df[dfs.source_records_df["record_id"] == rec_b_id]
    ...
    if req.action.lower() == "reject":
        ...
        dfs.audit_logs_df = pd.concat([pd.DataFrame([new_log]), dfs.audit_logs_df], ignore_index=True)
        dfs.audit_logs_df.to_csv(get_file_path("audit_logs.csv"), index=False)
        ...
        dfs.source_records_df.loc[dfs.source_records_df["record_id"] == rec_b_id, "beneficiary_id"] = b_id_keep
        dfs.source_records_df.to_csv(get_file_path("source_records.csv"), index=False)

        b_a_df = dfs.beneficiaries_df[dfs.beneficiaries_df["beneficiary_id"] == b_id_keep]
        b_b_df = dfs.beneficiaries_df[dfs.beneficiaries_df["beneficiary_id"] == b_id_merge]
        ...
        if not b_a_df.empty and not b_b_df.empty:
            ...
            dfs.beneficiaries_df.loc[dfs.beneficiaries_df["beneficiary_id"] == b_id_keep, "programs"] = merged_progs
            dfs.beneficiaries_df.loc[dfs.beneficiaries_df["beneficiary_id"] == b_id_keep, "confidence"] = 100

            dfs.beneficiaries_df = dfs.beneficiaries_df[dfs.beneficiaries_df["beneficiary_id"] != b_id_merge]
            dfs.beneficiaries_df.to_csv(get_file_path("beneficiaries.csv"), index=False)
...
@router.get("/quality/dimensions", response_model=QualityDimensionsResponse, tags=["quality"])
def get_quality_dimensions(dfs = Depends(get_dataframes)):
    if dfs.quality_dimensions_df.empty:
    ...
    dims = []
    for _, row in dfs.quality_dimensions_df.iterrows():
...
@router.get("/quality/trend", response_model=TrendResponse, tags=["quality"])
def get_quality_trend(dfs = Depends(get_dataframes)):
    if dfs.source_records_df.empty:
        return TrendResponse(data=[100] * 30)

    df = dfs.source_records_df.copy()
...
@router.get("/anomalies", response_model=List[AnomalyItem], tags=["anomalies"])
def get_anomalies(dfs = Depends(get_dataframes)):
    if dfs.anomalies_df.empty:
        return []

    anomalies = []
    for _, row in dfs.anomalies_df.iterrows():
        ...
        if not dfs.source_records_df.empty:
            existing_ids = set(dfs.source_records_df["record_id"].astype(str))
            rec_list = [record_id for record_id in rec_list if record_id in existing_ids]
            if len(rec_list) < 2:
                duplicate_groups = dfs.source_records_df.groupby("beneficiary_id")["record_id"].apply(list)
                matching_group = next((ids for ids in duplicate_groups if len(ids) >= 2), None)
                rec_list = [str(record_id) for record_id in matching_group[:3]] if matching_group else dfs.source_records_df["record_id"].astype(str).head(2).tolist()
...
def get_audit_trail(dfs = Depends(get_dataframes)):
    if dfs.audit_logs_df.empty:
        return {}

    grouped = {}
    for _, row in dfs.audit_logs_df.iterrows():
...
@router.post("/governance/anonymise", response_model=GovernanceToggleResponse, tags=["governance"])
def toggle_kdpa_anonymisation(
    current_user: User = Depends(require_role(["admin", "manager"])),
    dfs = Depends(get_dataframes)
):
    global kdpa_masking_enabled
    ...
    dfs.audit_logs_df = pd.concat([pd.DataFrame([audit_entry]), dfs.audit_logs_df], ignore_index=True)
    dfs.audit_logs_df.to_csv(get_file_path("audit_logs.csv"), index=False)
...
@router.get("/financial/reconciliation", response_model=FinancialReconciliationSummary, tags=["financial"])
def get_financial_reconciliation(
    current_user: User = Depends(require_role(["admin", "manager"])),
    dfs = Depends(get_dataframes)
):
    del current_user
    if dfs.source_records_df.empty or dfs.beneficiaries_df.empty:
    ...
    raw_records = len(dfs.source_records_df)
    master_identities = len(dfs.beneficiaries_df)
    duplicates = max(raw_records - master_identities, 0)
    stipend = 7500.0
    at_risk = dfs.beneficiaries_df[dfs.beneficiaries_df["status"].isin(["Review", "Conflict", "Needs review"])]
    return FinancialReconciliationSummary(
...
@router.get("/financial/leakage-trend", response_model=List[LeakageTrendItem], tags=["financial"])
def get_financial_leakage_trend(current_user: User = Depends(require_role(["admin", "manager"])), dfs = Depends(get_dataframes)):
    prevented = get_financial_reconciliation(current_user=current_user, dfs=dfs).duplicate_leakage_prevented
...
@router.get("/financial/high-risk-payments", response_model=List[HighRiskPaymentItem], tags=["financial"])
def get_high_risk_payments(current_user: User = Depends(require_role(["admin", "manager"])), dfs = Depends(get_dataframes)):
    del current_user
    if dfs.beneficiaries_df.empty:
        return []
    at_risk = dfs.beneficiaries_df[dfs.beneficiaries_df["status"].isin(["Review", "Conflict", "Needs review"])].head(10)
...
@router.get("/financial/reconciliation/settings", response_model=ReconciliationSettingsResponse, tags=["financial"])
...
@router.post("/financial/reconciliation/settings", response_model=ReconciliationSettingsResponse, tags=["financial"])
def update_reconciliation_settings(req: ReconciliationSettingsRequest, current_user: User = Depends(require_role(["admin"])), dfs = Depends(get_dataframes)):
    global mismatch_threshold, holding_on_mismatch
    mismatch_threshold = req.mismatch_threshold
    holding_on_mismatch = req.holding_on_mismatch
    now = datetime.now()
    dfs.audit_logs_df = pd.concat([pd.DataFrame([{"time": now.strftime("%H:%M:%S"), "date": now.strftime("%Y-%m-%d"), "event": "Reconciliation Rules Updated", "detail": f"Mismatch threshold set to {mismatch_threshold}% by {current_user.full_name}.", "type": "finance", "user": current_user.username}]), dfs.audit_logs_df], ignore_index=True)
    dfs.audit_logs_df.to_csv(get_file_path("audit_logs.csv"), index=False)
    return ReconciliationSettingsResponse(mismatch_threshold=mismatch_threshold, holding_on_mismatch=holding_on_mismatch)
...
@router.post("/notifications/dispatch", response_model=NotificationResponse, tags=["notifications"])
def dispatch_notification(req: NotificationRequest, current_user: User = Depends(require_role(["admin", "manager"])), dfs = Depends(get_dataframes)):
    now = datetime.now()
    reference_id = req.reference_id or f"NTF-{now.strftime('%Y%m%d%H%M%S')}"
    dfs.audit_logs_df = pd.concat([pd.DataFrame([{"time": now.strftime("%H:%M:%S"), "date": now.strftime("%Y-%m-%d"), "event": "Notification Dispatched", "detail": f"Mock {req.channel.upper()} notification sent to {req.recipient} for {reference_id}.", "type": "notification", "user": current_user.username}]), dfs.audit_logs_df], ignore_index=True)
    dfs.audit_logs_df.to_csv(get_file_path("audit_logs.csv"), index=False)
